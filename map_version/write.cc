...
// 使用protobuf构建通讯录2.0
// 从磁盘中读取序列化后的文件，进行反序列化，并添加新的内容，然后重新序列化后写回磁盘。
...
void AddPeopelInfo(PeopleInfo* people_info_ptr)
{
	...
	cout << "请输入联系人公司地址: ";
	getline(cin, comp_address);
	address.set_comp_address(comp_address);
	...
	// 其他联系信息---oneof类型
	cout << "选择设置一个其他联系方式：1.qq   2.wechat";
	int other_select;
	cin >> other_select;
	cin.ignore(256, '\n');
	if(other_select == 1)
	...
	{
		cout << "选择错误，不设置该项" << endl;
	}

	// 备注信息---map类型
	for(int i = 1; ; ++i)
	{
		cout << "请输入备注" << i << "的标题(只输入回车完成备注添加)：";
		string remark_key;
